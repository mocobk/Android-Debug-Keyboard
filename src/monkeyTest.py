...
PATH = lambda p: p  # 原来的（下方lambda）路径打包后无法找到路径
...
def runnerPool(devices):
    try:
        ...
        os.popen("del /F /Q info").read()  # 删除持久化目录
    # devices_Pool = []
    _app = {"devices": devices, "match_num": 1}
    ...
    # pool.join()
    # 这里取消了原来的多进程
    if os.path.exists('./monkey.ini'):
        start(_app)
    else:
        print('%s 配置文件未找到，请先运行monkey命令' % os.path.join(os.path.abspath('./'), 'monkey.ini'))
...
def stop_monkey(device):
    result = os.popen('adb -s %s shell ps | findstr monkey' % device).read().strip()
    try:
        pid = result.split()[1]
        os.popen('adb -s %s shell kill -9 %s' % (device, pid))
    except IndexError:
        pass
